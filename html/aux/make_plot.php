...
function make_memplot($plot_name, $y_data_array, $logxy, $x_size, $y_size, $num_fields, $sensor_name, $x_data, $row_length, $legend_names, $colors, $y_axis_title)
{
  ...
  if ($y_axis_title=="mem")
    ...
    {
      $graph->yaxis->title->Set("Percentage Active CPU");
    }
  $graph->xaxis->SetTitleMargin(50);
  $graph->yaxis->SetTitleMargin(70);
  $graph->xaxis->SetLabelAngle(90);
  ...
  $color_array = array("rd"=>"red", "or"=>"orange", "yl"=>"yellow", "gr"=>"green", "bl"=>"cyan", "vi"=>"darkviolet", "pk"=>"deeppink", "br"=>"saddlebrown");
  ...
  for( $i=0; $i<($num_fields-1); $i++)
      ...
      for($j=0; $j<$row_length; $j++)
	{
      ...
      $lineplot = new ScatterPlot($blocktype,$x_data);
      ...
      $lineplot_name=$legend_names[($i+1)];
      $lineplot -> SetLegend($lineplot_name);
  ...
  if($y_axis_title=="mem")
    ...
    {
      $graph->legend->Pos(0.87,0.73,'right','center');
    }
